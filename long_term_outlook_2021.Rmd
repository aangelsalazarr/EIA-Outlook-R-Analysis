...
# allows us to export editable graphs for powerpoint
library(here)
library(glue)
library(officer)
library(rvg)
...
```{r visualizing our new master df, fig.width=15, fig.height=10}
...
master_df %>%
  ...
  labs(x = "Year", y = "MMmtCO2/capita", title = "US Per-Capita CO2 Emissions", 
       ...
       color = "Economic Scenarios")
```


```{r visualizing sector emissions based on economic growth, fig.width = 20, fig.height=15}
master_df %>%
  filter(str_detect(name, "Commercial|Industrial|Residential|Transportation")) %>%
